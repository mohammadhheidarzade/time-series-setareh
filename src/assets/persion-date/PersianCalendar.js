...
function DateTime(ticks) {
    ...
    return { Year: DateTime_GetDatePart(dateDate, DatePartYear), Month: DateTime_GetDatePart(dateDate, DatePartMonth), Hour: GetHour({ Ticks: ticks }), Minute: GetMinute({ Ticks: ticks }), Second: GetSecond({ Ticks: ticks }), Ticks: ticks, Day: DateTime_GetDatePart(dateDate) };
}

function GetHour(time) {
    return parseInt((time.Ticks / TicksPerHour) % 24);
}

function GetSecond(time) {
    return parseInt((time.Ticks / TicksPerSecond) % 60);
}

function GetMilliseconds(time) {
    return parseFloat((time.Ticks / TicksPerMillisecond) % 1000);
}

function GetMinute(time) {
    return parseInt((time.Ticks / TicksPerMinute) % 60);
}
